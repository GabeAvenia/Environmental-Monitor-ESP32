...
bool SHT41Sensor::updateReadings() {
    if (!connected) {
        logError("Attempted to read from disconnected sensor: " + name);
        return false;
    }
    ...
    if (!sht4.getEvent(&humidity, &temp)) {
        logError("Failed to read from SHT41 sensor: " + name);
        connected = false;
        return false;
    }
...
void* SHT41Sensor::getInterface(InterfaceType type) {
    switch (type) {
        case InterfaceType::TEMPERATURE:
            return static_cast<ITemperatureSensor*>(this);
        case InterfaceType::HUMIDITY:
            return static_cast<IHumiditySensor*>(this);
        default:
            return nullptr;
    }
}
